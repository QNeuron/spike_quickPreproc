...
%           - savePath : str. where the data will be saved. Def : dataPath.
%
% Outputs :
...
if isnumeric(opt.savePath),
    opt.savePath = opt.dataPath;
end
...
if isfield(grid,'stimFiles'),
stimNames = grid.stimFiles;
else
    stimNames = grid.stimGrid;
end
expInfo.grid = grid;
expInfo.exp = expt;
...
if nSweep ~= NSweep,
    warning('The recording session has been interrupted.')
end
...
%             ,'waveforms', 'expInfo', 'abscissa','-v7.3');
        spikePointer = matfile(fullfile(opt.savePath,sprintf('spikes_%s_P%d_N%d_%s',expInfo.exp.userName,expInfo.exp.penetrationNum,expInfo.exp.exptNum,expInfo.grid.saveName)),'writable',true);
        ...
        waveformPointer = matfile(fullfile(opt.savePath,sprintf('spikes_waveforms_%s_P%d_N%d_%s',expInfo.exp.userName,expInfo.exp.penetrationNum,expInfo.exp.exptNum,expInfo.grid.saveName)),'writable',true);
...
%                     waveforms = [waveforms; Twaveforms];
                      ...
                      waveformPointer.waveforms(idx(1):idx(end),:) = Twaveforms;
                      TspikeMat = zeros(5000,5);
                      Twaveforms = zeros(5000,waveFormBin);
                      idx = idx+5000;
...
%     waveforms = waveforms(1:end-(5000-c+1),:);
    ...
    waveformPointer.waveforms = waveformPointer.waveforms(1:end-(5000-c+1),:);
    
else % parfor loop
    %     poolobj = parpool;
    spikeMat = cell(length(SelCh),1);
    waveforms = cell(length(SelCh),1);
    
    for ch = SelCh,
        TspikeMat = cell(length(data(1).AP),1);
        Twaveforms = cell(length(data(1).AP),1);
        parfor s = 1:length(data(1).AP), % Extract AP from the loaded set
            % trigger spikes
            [ bin_peak_max, val_peak_max, ~, peak_abs ] = peak_detector_general(data(ch).AP{s}, 'thr', thresholds(ch));
            ...
            for i = 1:length(bin_peak_max),
                TspikeMat{s}(i,1:5) = [(bin_peak_max(i)+(s-1)*sweepLength)/sr bin_peak_max(i)/sr grid.randomisedGrid(s) s ch];
                ind = bin_peak_max(i)-floor(waveFormBin/2):bin_peak_max(i)+ceil(waveFormBin/2)-1;
                Twaveforms{s}(i,1:waveFormBin) = data(ch).AP{s}(ind);
                
            end
            
        end
        waveforms{c,1} = zeros([sum(cellfun(@(x)(size(x,1)),TspikeMat)),waveFormBin]);
        waveforms{c,1} = cell2mat(Twaveforms);
